...
class RerankerCrossModel:
    ...
    def _resolve_model_path(self, model_name_or_path: str) -> str:
        """尝试解析相对模型路径，如果本地路径存在则返回绝对路径。"""
        if os.path.exists(model_name_or_path):
            return os.path.abspath(model_name_or_path)

        path_from_script = os.path.abspath(
            os.path.join(os.path.dirname(__file__), model_name_or_path)
        )
        if os.path.exists(path_from_script):
            return path_from_script

        # 保留原始参数，便于加载远程模型或让上层决定是否可用
...
if __name__ == "__main__":
    ...
    reranker = RerankerCrossModel(
        ...
        # 若未下载本地模型，可直接使用远程模型名称让 HuggingFace 自动下载缓存
        # model_name_or_path="BAAI/bge-reranker-base",  # 远程模型名称
        model_name_or_path="BAAI/bge-reranker-large",  # 远程模型名称，首次运行会自动下载模型缓存
