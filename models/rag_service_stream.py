...
from langchain_embedding import initialize_embedding_model
from langchain_llm import langchain_qwen_llm
from reranker_model import RerankerCrossModel
# 1. 配置日志输出格式和级别（如果项目里其他地方配置过了，这一行可以不写）
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')

# 2. 定义并初始化 logger 对象（解决 NameError 的核心）
...
class RAGService:
    ...
    def __init__(self,
        ...
        # )
        self._chat_history_manager = ChatMessageHistory()
        self.k = 50
    ...
    def get_answer_stream(self, question: str) -> Generator[str, None, None]:
        ...
        for msg in self._chat_history_manager.messages[-2 * self.k:]:
